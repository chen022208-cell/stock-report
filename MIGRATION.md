...
> 這份是「把 10 支雲端 Routine 從舊帳號（Binchilin）搬到新 Claude 帳號」的完整
> 執行清單。舊帳號的 live 設定 2026-09-07 抓下來，prompt 原文照貼即可。
> GitHub / Google 表單 / cron-job.org 那側都不用動。

---

## 0. 觀念

| 元件 | 吃 Claude 額度 | 換帳號要動 |
|---|---|---|
| 下面 10 支 Routine | **是**（Pro/Max） | **全部重建** |
| GitHub Actions（intraday.yml、daily-notify.yml、deep-report-notify.yml、Pages） | 否 | 不動 |
| cron-job.org（每天 08:45 觸發盤中迴圈） | 否 | 不動 |
| Google Apps Script（表單送出觸發） | 否 | 只改 `TRIGGER_URL` 指向新帳號的 routine |
| GitHub Secrets（DISCORD_WEBHOOK_URL 等）、網站本體 | 否 | 不動 |

**不需要任何計量 API key**（`ANTHROPIC_API_KEY` 只是 report.yml 手動備援）。
...
2. **雲端環境要能 push 到 `chen022208-cell/stock-report`**：
...
1. 照第 3 節，用 `/schedule` 或 https://claude.ai/code/routines 逐支重建 10 支。
   每支要給：**名稱 / cron / model / git repo / allowed_tools / prompt（整段照貼）**。
2. 重建「台股使用者研究提交處理」後 → 進那支 → **+ Add trigger → API** → 拿到
   新的 URL＋token → 更新 Google Apps Script 指令碼屬性 `TRIGGER_URL` /
   `TRIGGER_TOKEN`（見第 4 節）。
3. 「台股盤中焦點股深度快報」的 GitHub push webhook **先不要重建**（見第 4 節說明）。
4. 每支 `/schedule` 按一次 **Run now** 測試；早報／盤後遇到台股休市日會正常略過。
5. 全部確認 OK → 回**舊帳號** https://claude.ai/code/routines → 10 支全部 **Disable**
   （API 只能停用不能刪）。**一定要關**，否則兩帳號同時跑、互相 git 衝突、舊帳號繼續燒。

---

## 3. 10 支 Routine 完整規格

共通：**git source** = `https://github.com/chen022208-cell/stock-report`；
**persist_session** = false；環境用新帳號自己的。

### 3.1 台股每日早報

- 舊帳號 id（僅供對照，不用填）：`trig_01R9kev8w4MQvozhPWEsv61g`
- **cron（UTC）**：`0 23 * * 0-4`　＝ 台灣 週一–週五 07:00（週日–四 23:00 UTC）
- **model**：(帳號預設)
...
- 舊帳號 id（僅供對照，不用填）：`trig_01E5e1rMVnZk6JbxSpgxXu4K`
- **cron（UTC）**：`0 10 * * 1-5`　＝ 台灣 平日 18:00
- **model**：(帳號預設)
- **allowed_tools**：`preset:default, Task, Bash, Glob, Grep, Read, Edit, MultiEdit, Write, NotebookEdit, WebFetch, TodoWrite, WebSearch, BashOutput, KillBash, Skill, Tmux, Monitor, SendUserFile, REPL`
- **prompt**：
...
- 舊帳號 id（僅供對照，不用填）：`trig_01HTffRTdsBaegyH5EiyRryw`
- **cron（UTC）**：`33 * * * *`　＝ 台灣 每小時 33 分
...
- 舊帳號 id（僅供對照，不用填）：`trig_01WCoQ9PpAkkwE6PgAWEzR9H`
- **cron（UTC）**：`8 * * * *`　＝ 台灣 每小時 08 分
- **model**：claude-sonnet-5
- **allowed_tools**：`preset:default, Task, Bash, Glob, Grep, Read, Edit, MultiEdit, Write, NotebookEdit, WebFetch, TodoWrite, WebSearch, BashOutput, KillBash, Skill, Tmux, Monitor, SendUserFile, REPL`
- ⚠️ 這支還要在網頁加一個 **API trigger**（見第 4 節）
- **prompt**：

```
你正在為「盤後快訊」台股報告網站（chen022208-cell/stock-report）跑「使用者研究提交處理」。把使用者透過 GitHub Issue（research-submission 標籤）、Google 表單提交的研究內容、以及主題點播（標題帶「[主題點播]」前綴或 topic-request 標籤），做分析＋驗證後整理進網站。跑順且沒有新提交就安靜結束、不要通知使用者。

【執行步驟】
1. 工作目錄若還沒有 repo，就 `git clone https://github.com/chen022208-cell/stock-report` 再 `cd stock-report`；已存在就 `cd` 進去執行 `git pull --rebase origin main`。
2. `pip install -r requirements.txt`（裝過會很快跳過）。主題點播報告會產 PDF，另外裝 weasyprint（不在 requirements，因為本機 Windows 裝不起來）：`pip install weasyprint`；裝完用 `python -c "import weasyprint; print('ok')"` 確認。若 import 仍失敗（缺 pango/cairo 系統庫），程式會自動跳過 PDF、只留 HTML，不算錯但盡量裝起來。
3. 檢查 `gh auth status`：能用就用（讓程式能讀 GitHub Issue）；不能用也沒關係，Google 表單那條路（config.yaml 的 google_sheet_csv_url，公開 CSV）不需要 gh。
4. 用 Bash 工具、背景執行（run_in_background: true）啟動：
   ...
   - 主題點播（[主題點播] 前綴）走的是另一條線：使用者只給一個主題，系統自己上網查證後寫一份深度報告（個股「在做什麼」只能用查到的，不可用名稱推測；歷史事故：7686 捷立康被寫成 PCB 廠），sources 至少 2 個外部來源，查不到就不產出。
5. 這支程式在需要 LLM 回覆的地方，會把請求寫成 `agent_llm_queue/<uuid>.request.json`（內容是 `{"system": ..., "user": ..., "max_tokens": ...}`），然後卡住等待對應的 `agent_llm_queue/<uuid>.response.txt` 出現。你必須在背景程序執行期間，用一個輪詢迴圈（每 5-10 秒 `ls agent_llm_queue/*.request.json` 一次）偵測新出現的 `.request.json`：
   ...
   - 繼續輪詢直到背景的 `python -m src.main` 程序結束（用 BashOutput 確認已結束、沒有殘留未回覆的 `.request.json`）
   - 單一請求最多等 20 分鐘會逾時，所以輪詢間隔不要太長
6. 程式會把處理結果留言在對應的 GitHub Issue 並關閉它（若走 gh 那條路）。主題點播的 PDF 已由程式自動產（render_topic_pdf，weasyprint）；若終端印 weasyprint 不可用、你這輪才裝起來，就對這批新產的 docs/analysis/<日期>-<slug>.html 手動補一份同名 .pdf（`weasyprint docs/analysis/X.html docs/analysis/X.pdf` 或 `chromium --headless --print-to-pdf`），再跑一次 `python -m src.main site` 讓頁面出現「下載 PDF」連結。
7. 確認背景程序成功結束後：
   - `git add -A`
   ...
   - docs/_notify_topic.json 一進 main，daily-notify.yml 會自動發 Discord（主題＋摘要＋ PDF 連結）

【重要】
- 大多數時候沒有新提交，程式不會有任何 git 變更，這時安靜結束、不要通知使用者。
- 只有下列情況才用 PushNotification：`python -m src.main research` 執行失敗（有 traceback、非正常結束）、輪詢中 `.request.json` 長時間沒被你處理到、或 git push 重試 3 次後仍失敗。gh 不可用、weasyprint 裝不起來都不算失敗。
- 絕對不要去動 `docs/weekly/`、`docs/monthly-deep/` 底下的檔案，那是另外兩個獨立的 Routine 在維護。
```
...
- 舊帳號 id（僅供對照，不用填）：`trig_01E5mQG59kmfAewV1V79BoBY`
- **cron（UTC）**：`0 3 * * *`　＝ 台灣 每天 11:00
- **model**：claude-sonnet-5
...
- 舊帳號 id（僅供對照，不用填）：`trig_016UxxY7BrSJJdvmbvcZE3ae`
- **cron（UTC）**：`30 11 * * 1-5`　＝ 台灣 平日 19:30
- **model**：claude-sonnet-5
- **allowed_tools**：`Bash, BashOutput, KillBash, Read, Write, Edit, Glob, Grep, WebSearch, WebFetch, TodoWrite`
- ⚠️ **可考慮不建**：功能已併進「使用者研究提交處理」，這支等於備援
- **prompt**：

```
你正在為「盤後快訊」台股報告網站（chen022208-cell/stock-report）跑「主題點播深度報告」。使用者在提交研究頁點播一個主題（GitHub Issue，標籤 topic-request，標題以「[主題點播]」開頭），
...
- 舊帳號 id（僅供對照，不用填）：`trig_011ofCnhpdNwX9gG6dqfVsKU`
- **cron（UTC）**：`0 4 * * *`　＝ 台灣 每天 12:00
- **model**：claude-sonnet-5
- **allowed_tools**：`Bash, BashOutput, KillBash, Read, Write, Edit, Glob, Grep, WebSearch, WebFetch, TodoWrite`
- **prompt**：
...
4. 輪詢（每 5-10 秒 `ls agent_llm_queue/*.request.json`）偵測 request：
   ...
   - 寫進 `agent_llm_queue/<同一個 uuid>.response.txt`。用 BashOutput 確認程式已結束。
...
- 舊帳號 id（僅供對照，不用填）：`trig_01HpFbfTcMzrwPn2LdBG3Smh`
- **cron（UTC）**：`0 3 * * 1-5`　＝ 台灣 平日 11:00
- **model**：claude-sonnet-5
- **allowed_tools**：`Bash, BashOutput, KillBash, Read, Write, Edit, Glob, Grep, WebSearch, WebFetch, TodoWrite`
- ⚠️ 這支舊帳號還有一個 **GitHub push webhook**——**先不要重建**（見第 4 節）
- **prompt**：
...
### 3.9 台股週報：全球總經＋台股深度研究

- 舊帳號 id（僅供對照，不用填）：`trig_01FkVN6yTac8U5qmo6npYQuY`
- **cron（UTC）**：`0 12 * * 5`　＝ 台灣 週五 20:00
- **model**：claude-sonnet-5
...
寫作要求：分析文章形式、有論述脈絡，不是條列摘要；明確區分「已驗證的事實」與「你的推論」；完全不要用投資建議的語氣（不是推薦買賣，是資訊整理）；重要數據盡量註明來源。抓資料不要抓到今天當週市場尚未發生的內容。
...
### 3.10 台股深度月報：全球總經＋台股結構＋焦點個股

- 舊帳號 id（僅供對照，不用填）：`trig_01MVn2uDUP28mygm1wEjfHBj`
- **cron（UTC）**：`0 0 1 * *`　＝ 台灣 每月 1 號 08:00
- **model**：claude-sonnet-5
- **allowed_tools**：`Bash, Read, Write, Edit, Glob, Grep, WebSearch, WebFetch`
- **prompt**：
...
## 4. 觸發器（webhook / API trigger）

### 4.1 使用者研究提交處理 → API trigger（要建）

讓 Google 表單送出後幾分鐘內就跑（不用等 `8 * * * *` 那班 cron）。

1. 新帳號 → 那支 routine → Edit → **+ Add trigger** → **API**。
2. 拿到觸發網址（形如 `https://api.anthropic.com/v1/claude_code/routines/<新id>/fire`）＋ token。
3. Google 試算表（表單回應那份）→ 擴充功能 → Apps Script → **專案設定 → 指令碼屬性**：
   - `TRIGGER_URL` = 上面那個網址
   - `TRIGGER_TOKEN` = 上面那個 token
   （`onFormSubmit` 程式碼不用改，範本在舊 `ROUTINES.md` 第 8 節；重點：headers 要帶
   `anthropic-version: 2023-06-01`，不然 400。）
4. Apps Script 執行一次 `onFormSubmit`，看到 `觸發回應 200 … "type":"routine_fire"` 就對。
5. **舊帳號**那支上的舊 API trigger 不用特別處理（停用 routine 後就不會被打）。

### 4.2 盤中焦點股深度快報 → GitHub push webhook（建議不要重建）

舊帳號這支有一個「GitHub push」webhook，本意是「盤中一有新 A 級訊號就秒觸發」，
但它**對 repo 每一次 push 都觸發**（無法只綁分支／檔案），結果被各種 commit 打了幾十次、
全部空跑。新帳號**先只留 cron**（`0 3 * * 1-5` = 台灣平日 11:00 盤中掃一次）。
真的要盤中即時，之後再單獨設計「只在 intraday-data 分支有新訊號時」才觸發的機制。

---

## 5. 關掉舊帳號的 10 支

https://claude.ai/code/routines（**舊帳號登入**）→ 每支點進去 → **Disable**（或 Delete）。
舊帳號 id 清單：

- `trig_01R9kev8w4MQvozhPWEsv61g`　台股每日早報
- `trig_01E5e1rMVnZk6JbxSpgxXu4K`　台股每日盤後
- `trig_01HTffRTdsBaegyH5EiyRryw`　台股即時快訊監控
- `trig_01WCoQ9PpAkkwE6PgAWEzR9H`　台股使用者研究提交處理
- `trig_01E5mQG59kmfAewV1V79BoBY`　台股個股逐檔查證（每天 10 檔）
- `trig_016UxxY7BrSJJdvmbvcZE3ae`　台股主題點播深度報告
- `trig_011ofCnhpdNwX9gG6dqfVsKU`　台股產業深度分析
- `trig_01HpFbfTcMzrwPn2LdBG3Smh`　台股盤中焦點股深度快報
- `trig_01FkVN6yTac8U5qmo6npYQuY`　台股週報：全球總經＋台股深度研究
- `trig_01MVn2uDUP28mygm1wEjfHBj`　台股深度月報：全球總經＋台股結構＋焦點個股
...
- 「個股逐檔查證」與「盤中焦點股深度快報」cron 都是 `0 3`（台灣 11:00）——不同 routine、
  各跑各的，不影響，知道就好。
- 「產業深度分析」`0 4`、「使用者研究提交處理」`8 * * * *` 也都在盤中時段，同理。
