...
# ── 每日評論 ───────────────────────────────────────────
COMMENTARY_SYSTEM = """你是台股資深分析師，替一位有程式與投資背景的讀者撰寫盤後短評。

要求：
- 繁體中文，2-4 句，直接寫結論不要鋪陳
- 解讀數字之間的關聯（例如資金輪動、內外資態度差異），不要只複述數字
...
def market_commentary(payload: dict) -> str:
    ...
    try:
        return _call(COMMENTARY_SYSTEM,
                     json.dumps(payload, ensure_ascii=False, indent=2), 800).strip()
    except Exception as exc:
