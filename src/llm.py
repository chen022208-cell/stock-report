...
- 每個具體數字（營收、年增率、EPS、資本額、法人持股）都必須是你**實際查到**的，
  查不到就整句不要寫。**絕對不要估算、不要沿用其他公司的數字、不要編**。
- sources 必填，至少 1 個是申報值以外的外部來源。查不到可靠來源就不要寫那一檔。

【語氣】
描述「現在是什麼狀態、要觀察什麼」，不要寫成給讀者的買賣指令。
可以寫「站上月線後沿短均走強」「量能連三日放大」；
不要寫「建議買進」「逢低承接」「目標價 X 元」。
推論性的判斷一律標「（推論）」，跟查到的事實分開。

【每檔輸出這六段】
1. decision：{"stance": 目前的多空結構狀態一句話, "thesis": 核心邏輯一句話——
   這家公司在做什麼、為什麼現在受關注}
2. themes：題材面，2～5 條 [{"label": 小標, "body": 說明}]。
   只放查得到的：具體事件、合作案、產品進展、掛牌狀態。查不到就少放幾條。
3. technical：技術面 1～2 句，用附上的量價訊號描述結構。
4. chips：籌碼面，1～3 條 [{"label": 小標, "body": 說明}]。
   資本額、法人／大戶持股這類數字**查得到才寫**，查不到就只寫觀察不到的部分。
5. fundamental：{"revenue": 營收與獲利實際數字（查不到就空字串）,
   "outlook": 後續兩季的觀察重點（標「（推論）」）, "consensus": 查得到的市場共識或空字串}
6. peers：{"compare": 同業競合位置, "risks": 風險提示}
...
只輸出 JSON，不要 markdown fence，key 是股票代號；查不到就不要放：
{
  "1234": {
    ...
    "technical": "...",
    "chips": [{"label": "...", "body": "..."}],
    "fundamental": {"revenue": "...", "outlook": "...", "consensus": "..."},
    "peers": {"compare": "...", "risks": "..."},
    "sources": ["https://...", "..."]
  }
}"""
