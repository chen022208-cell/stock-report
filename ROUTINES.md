...
- **cron**：4 個錯開的 trigger `2 * * * *`／`17 * * * *`／`32 * * * *`／`47 * * * *`
  （實際延遲約 15 分鐘）
...
- **cron**：4 個錯開的 trigger `8 * * * *`／`23 * * * *`／`38 * * * *`／`53 * * * *`
  （實際延遲約 15 分鐘）
...
⚠️ **平台硬限制：Claude Routine 的每個 schedule 最多一小時一次。**
在建立／編輯畫面填 `*/5 * * * *` 會被擋下並顯示
`Schedules must run at most once per hour.`（2026-09-07 實際踩到）。

繞過方式是**同一支 Routine 掛多個錯開的 trigger**（編輯畫面下方的
`+ Add another trigger`）——限制是「每個 schedule 一小時一次」，不是「每支
Routine 一小時一次」。掛 4 個就等於每 15 分鐘：

- 即時快訊監控：`2 * * * *`／`17 * * * *`／`32 * * * *`／`47 * * * *`
- 使用者提交處理：`8 * * * *`／`23 * * * *`／`38 * * * *`／`53 * * * *`

兩支的分鐘數刻意錯開，避免同時醒來搶同一個 repo 的 rebase。介面另有提示
`Runs are staggered by a few minutes to spread server load.`，實際觸發還會再飄
幾分鐘，所以不要把間隔抓得太剛好。

再密也沒有意義：CCR session 光是 clone repo ＋ pip install 就要一兩分鐘。

**背景回填**——刻意限速，不屬於「及時」也不該調快：
個股逐檔查證（每天 40 檔，補滿全市場 2345 檔）、產業深度分析（每天 6 類，補滿 37 類）。
這兩支沒有「事件」可以反應，速度上限來自查證品質與訂閱額度；再密集叫醒只會
...
**額度成本**：兩支各掛 4 個 trigger，合計一天約 192 次執行（多數是空跑後立刻結束）。
如果覺得額度吃緊，先把「使用者提交處理」減成 2 個 trigger（半小時一次）——
快訊那支比較值得留住 15 分鐘的密度。
...
| 即時快訊監控 | 4 trigger `2/17/32/47 * * * *` | 約每 15 分鐘 |
| 使用者提交處理（研究提交＋主題點播，全部入口） | 4 trigger `8/23/38/53 * * * *` | 約每 15 分鐘 |
| ~~主題點播深度報告~~（已併入上一支，可停用） | — | — |
| 產業深度分析 | `0 4 * * *` | 每天 12:00 |
| 個股逐檔查證 | `0 3 * * *` | 每天 11:00 |
| 週報 | `0 12 * * 5` | 週五 20:00 |
