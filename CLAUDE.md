...
- **每日早報／盤後**：Claude Code Routine「台股每日早報」「台股每日盤後」驅動，
  ...
  **凡是會 push 的 Routine，job_config 都要掛 git source
  ...
  不要自己猜 JSON 格式。
  ⚠️ **`LLM_AGENT_MODE` 的熔斷（2026-09-08 加，別拿掉）**。雲端 CCR session 是
  「一邊跑 Python、一邊自己扮演 LLM 服務 `agent_llm_queue/`」。session 的 Monitor
  一逾時就不再回覆佇列，但 Python 那端還活著——而 `run_evening` 有七、八個 LLM
  呼叫點（題材聚類、新聞分類、個股深度分析、評論，再加每個題材一篇深度報告），
  **每一個都各自等滿 20 分鐘**才被 `_safe()` 接住，下一個再等 20 分鐘，累加起來
  好幾個小時跑不完。2026-09-07 盤後就是這樣從 20:04 idle 到天亮，熱力圖與評分
  停在 09-04，最後是本機補跑的。現在 `llm._call_via_agent_queue()` 第一次逾時就
  設 `_agent_down_reason`，之後每個呼叫立刻拋錯讓 `_safe()` 降級：一輪最多只浪費
  一次逾時（20 分鐘），而不是呼叫點數 × 20 分鐘。
  **熱力圖／評分／籌碼／漲幅榜全是規則計算，沒有 LLM 照樣產得出當天資料**（已實測），
  所以降級後報告仍然會更新，只是少掉個股解讀那一層——那層本來就必須有查證來源，
  沒有就不該寫。逾時秒數可用 `LLM_AGENT_TIMEOUT` 環境變數覆蓋（測試用）。
...
- **推播門檻看漲幅，不看分數**：`config.yaml` 的 `intraday.alert_change_pct`
  （預設 3.0）。通過漏斗的 A／B 級標的，當日漲幅一突破就推一次。
  用 `intraday_signals.json` 裡的 `alerted` 去重，一檔一天只推一次。
  ⚠️ **這條跟 `deep_report_*` 是兩回事，不要合併**：`new_codes` 驅動的是深度
  快報、有每日 10 篇上限，把 3% 那批併進去會被 B 級瞬間燒光。
- **推播訊息每檔之間用 `_____` 分隔**（使用者指定），`_write_alert()` 產生。
  它吃的是 `result["tiers"]` 裡的列（A／B 都可以），不是舊的 {code: 訊號} 字典。
- **個股快報改成六段式「核心關鍵投資報告」**：投資決策／題材面／技術面／
  籌碼面／基本面與未來兩季／同業比較與風險提示。
  內容存在 `intraday_reports.swot` 這個 **JSON 欄位**裡——欄位名稱沿用舊的、
  沒有改 schema，實際語意已經是「報告內容」。舊報告仍是 SWOT 四個 key，
  `templates/analysis.html` 兩種都認得（先看有沒有六段的 key，沒有才走 SWOT），
  所以既有頁面不會壞。
  **查證鐵則不變**：數字（營收、EPS、資本額、法人持股）必須實際查到才能寫，
  查不到整句不寫，不估算不沿用；sources 至少 1 個外部來源。語氣描述「現在是
  什麼狀態、要觀察什麼」，不寫買賣指令與目標價。

## ⚠️ 漲跌幅只有一條規則（2026-09-18 定案）

**用資料來源自己給的漲跌，不要自己推算。** 除權息、減資日的「參考價」不是昨天
收盤價，拿昨收去除就會錯。來源優先順序：

| 市場 | 漲跌來源 |
|---|---|
| 上市 | 證交所官方「漲跌價差」（後端 STOCK_DAY_ALL／前端 STOCK_DAY 第 8 欄） |
| 上櫃 | 櫃買官方 `Change`（`tpex.fetch_daily_quotes`） |
| 興櫃 | **Yahoo**（`yahoo.fetch_quotes`，spark 端點的現價／昨收）——櫃買沒有漲跌欄 |
| 盤中 | MIS 的 `y`（官方參考價） |
| 盤中收盤後 | 價格用 Yahoo（反映 13:30 集合競價），漲跌仍對 MIS 的 `y` |

踩過的坑：
- 興櫃以前拿「前日均價」當基準 → 新掛牌整排對不上（7942 頁面 +1.34%，實際 -1.76%）。
- 從 Yahoo 日線前一根 K 推昨收也錯（7942 日線那根 505，Yahoo 頁面昨收 503）。
- **上櫃不能用 Yahoo**：全市場 1949 檔比對，7 檔上櫃不一致，6236 官方 -9.86%、
  Yahoo -16.01%——超過 10% 漲跌限制，Yahoo 的昨收沒有調整參考價。
- MIS 的上櫃收盤後常停在 13:29 那筆（5465 顯示 26.6，官方收盤 26.7）。
- Yahoo 帶漲跌% 的 v7/quote 要憑證（401）；spark 批次上限 20 檔（30 檔以上回 400）。

驗證方式：`twse.fetch_daily_quotes()`＋`tpex.fetch_daily_quotes()` 對 `yahoo.fetch_quotes()`
全市場逐檔比，價格應全部相同；漲跌不同的一定是參考價調整日，以官方為準。
