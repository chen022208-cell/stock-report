...
3. **`themes` / `desc` / `swot` — 判讀**：題材是本站題材知識庫的歸類（只有
   ~156 檔被歸過類，**不硬把每檔都塞進題材**，沒歸類的前端就明寫「尚未歸入
   任何題材」）；`desc`／`swot` 由 `llm.company_swot_batch()` 產出，**輸入一定要
   帶上第 1 層的 `business`**，prompt 明令 `company_desc` 只能是營業項目的白話
   改寫、禁止用股票名稱或產業分類推測、沒有 `business` 的個股整檔不送也不寫。
